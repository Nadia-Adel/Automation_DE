...
public class TC007_ActualCost_ALM extends MobileTestBase{

	//Upload Required CMS and clear app data via the new reset btn 

	@Test(priority = 1)
	public void Step1_uploadRequiredCMS() throws IOException, InterruptedException  {
		CMDFeatures.RunBashScriptToUpdateCMSFile_Parameterized(CMSConfigFilesReader.getRequiredPath("bashScriptToUploadCMS_Path"), CMSConfigFilesReader.getRequiredPath("ActualCost_NoEligibleItemForPostpaid"));
	...
	public void Step4_ValidateSquareTile() {
		...
		assertFalse(Mobile_CommonActions_Set.checkPresenceOfElement(homePageObject.squareTileHeadline));
		assertFalse(Mobile_CommonActions_Set.checkPresenceOfElement(homePageObject.squareTileSubline));
		assertFalse(Mobile_CommonActions_Set.checkPresenceOfElement(homePageObject.squareTileIcon));
	}
